...
const Left = ({setFrom, from, setInput, input, setImg, img, min}) => {

    const [isOpen, setOpen] = useState(false);
    const handleOpen = () => setOpen(!isOpen); 
    
    const [inputDirty, setInputDirty] = useState(false)
    const [inputError, setInputError] = useState('enter the amount')

    const inputHandler = (e) => {
        setInput(e.target.value)
        if (input < min) {
            setInputError(`Min amount ${min}`)
        } else {
            setInputError("")
        }
    }
    ...
    return (
        <div className={isOpen? "activ_crypto_menu" : "prob_left"}>
                {(inputDirty && inputError) && <div className="error">{inputError}</div>}
                <div className="input_form">
                    <input onBlur={e => blurHandler(e)} onChange={e =>inputHandler(e)} name="input_name" value={input} />
                ...
                <div className="dropdown">
                    ...
                    <button id='buttonChange' className="drop" onClick={handleOpen}>
